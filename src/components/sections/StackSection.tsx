      ...
      <div className="stack-grid">
        {stackGroups.map((group, index) => {
          ...
          <article className="stack-group" key={group.label}>
            ...
            {group.items.map((item) => <div className="stack-item" key={item}><span className="stack-item__identity"><StackLogo name={item} /><span>{item}</span></span><ArrowUpRight size={14} /></div>)}
          </article>
        )})}
      </div>
    </section>
